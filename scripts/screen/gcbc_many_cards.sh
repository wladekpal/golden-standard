...
run_job() {
  ...
  local run_exp_name="gcbc_${learning_rate}_lr_${discount}_gamma_${batch_size}_bs"
  ...
  (
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" uv run --active src/train.py \
      ...
      --exp.seed "$seed" \
      --exp.project "gcbc_golden_standard" \
      --exp.epochs 50 \
      --exp.gif_every 10 \
      --agent.alpha 0.1 \
