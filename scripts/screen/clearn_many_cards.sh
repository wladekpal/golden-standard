...
run_job() {
  ...
  local run_exp_name="clearn_td_${learning_rate}_lr_${discount}_gamma_${batch_size}_bs"
  local discounted_mc_flag=()

  if [ "$use_discounted_mc_rewards" = true ]; then
    run_exp_name="clearn_mc_${learning_rate}_lr_${discount}_gamma_${batch_size}_bs"
    discounted_mc_flag+=(--agent.use_discounted_mc_rewards)
  fi
