...
from impls.utils.networks import GCActor, GCBilinearValue, GCDiscreteActor, GCDiscreteBilinearCritic, LogParam, GridStateEmbedding
...
class CRLSearchAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        return cls(rng, network=network, config=flax.core.FrozenDict(**config), grid_size=grid_size)
...
def get_config():
    config = ml_collections.FrozenConfigDict(
        dict(
            ...
            use_embeddings=False,  # Whether to use grid state embeddings.
            lr=3e-4,  # Learning rate.
            batch_size=256,  # Batch size.
            embed_dim=64,  # Embedding dimension for grid states.
            use_position_embeddings=True,  # Whether to use position embeddings.
