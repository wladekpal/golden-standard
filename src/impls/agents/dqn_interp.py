...
from impls.utils.networks import ensemblize, LogParam
...
class GCInterpDiscreteCritic(nn.Module):
    ...
    def setup(self):
        critic_cls = InterpolationThinkingCritic
        if self.ensemble:
...
class GCDQNInterpAgent(flax.struct.PyTreeNode):
    ...
    def sample_actions(
        ...
        if not self.config["discrete"]:
            raise NotImplementedError("GCDQNInterpAgent supports only discrete action spaces.")
        ...
        if self.config["action_sampling"] == "softmax":
            ...
            actions = jnp.where(
                probs < self.config.get("epsilon", 0.1),
                random_actions,
                greedy_actions,
            )
    ...
    def create(
        ...
        if not config["discrete"]:
            raise ValueError("GCDQNInterpAgent supports only discrete action spaces.")
        ...
        encoders = dict()
        encoder_name = config.get("encoder") or None
        if encoder_name:
