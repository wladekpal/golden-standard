...
state, action, goal = state[:,0].reshape(state.shape[0], -1), action[:,0].reshape(action.shape[0]), goal[:,0].reshape(goal.shape[0], -1) # Take only first timestep (original state shape (256, 9, 3, 3, 2))
